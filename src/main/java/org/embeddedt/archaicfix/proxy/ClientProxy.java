...
public class ClientProxy extends CommonProxy {
    ...
    public void onRenderOverlay(RenderGameOverlayEvent.Text event) {
        ...
        if (ArchaicConfig.showBlockDebugInfo && mc.objectMouseOver != null
                && mc.objectMouseOver.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
            if (!event.right.isEmpty()
                    && Objects.firstNonNull(event.right.get(event.right.size() - 1), "").length() > 0)
                event.right.add("");
            Block block = mc.theWorld.getBlock(mc.objectMouseOver.blockX, mc.objectMouseOver.blockY,
                    mc.objectMouseOver.blockZ);
            int meta = mc.theWorld.getBlockMetadata(mc.objectMouseOver.blockX, mc.objectMouseOver.blockY,
                    mc.objectMouseOver.blockZ);
        ...
        if (ArchaicConfig.modernizeF3Screen) {
            ...
            for (int i = 0; i < event.left.size() - 3; i++) {
                /*
                 * These checks should not be inefficient as most of the time the first one will
                 * already fail
                 */
                if (!hasReplacedXYZ && Objects.firstNonNull(event.left.get(i), "").startsWith("x:")
                    ...
                    hasReplacedXYZ = true;
                    int heading = MathHelper.floor_double((double) (mc.thePlayer.rotationYaw * 4.0F / 360.0F) + 0.5D)
                            & 3;
                    String heading_str;
                    switch (heading) {
                        case 0:
                    ...
                    event.left.set(i, String.format("XYZ: %.3f / %.5f / %.3f", mc.thePlayer.posX,
                    ...
                    event.left.set(i + 1, String.format("Block: %d %d %d [%d %d %d]", blockX, blockY, blockZ,
                    ...
                    event.left.set(i + 3,
                            String.format("Facing: %s (%s) (%.1f / %.1f)",
                                    Direction.directions[heading].toLowerCase(Locale.ROOT), heading_str,
            ...
            for (int x = 0; x < event.left.size(); x++) {
                ...
                Gui.drawRect(1, strY - 1, strX + fontrenderer.getStringWidth(msg) + 1,
                        strY + fontrenderer.FONT_HEIGHT - 1, rectColor);
